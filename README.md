...
final class KeycloakRoleMapper implements RoleMapperInterface
{
    ...
    {
        if (in_array('app-project-vet', $identity->roles, true)) {
            $user->syncRoles(['Veterinarian']);
        }

        if (in_array('app-project-client', $identity->roles, true)) {
...
| `06-app-project-integration-example.md` | Example integration in a modular Laravel system. |
